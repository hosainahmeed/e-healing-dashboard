...
const DriverRegistrationForm = () => {
  ...
  useEffect(() => {
    if (data && data.data) {
      ...
      // Set form values for document uploads
      form.setFieldsValue({
        ...populatedData,
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (imageFile) {
        ...
        // If editing existing driver, keep existing image
        submitFormData.append('profile_image', finalData.profile_image);
      } else {
        submitFormData.append('profile_image', finalData.profile_image);
      }
        ...
        if (
          ...
          if (finalData[key] && finalData[key].format) {
            ...
            if (newFiles.length > 0) {
              ...
              // No new file, check if existing file should be kept
      ...
      // Add ID if editing existing driver
      if (id) {
        submitFormData.append('id', id);
      }

      if (id) {
        const res = await updateDriver({ data: submitFormData }).unwrap();
        if (res?.success) {
  ...
  const handleDocumentChange =
    ...
    ({ fileList }) => {
      const updatedFileList = fileList.map((file) => {
        ...
        return file; // Keep existing files as is
      });
  ...
  return (
    <div className="w-full max-w-3xl mx-auto py-4">
      ...
      <Form
        ...
        form={form}
        layout="vertical"
        // Remove initialValues since we're setting them programmatically
      >
        {current === 0 && (
          <div>
            <div className="flex justify-center mb-6">
